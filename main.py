...
async def export_wav(data: ExportRequest):
    ...
    try:
        ...
        drum_stem = render_drums(
            ...
            bpm=data.bpm,
            swing=data.swing,
            mixer_channels=mixer_channels[:8],
        ...
        synth_stem = render_synth(
            ...
            bpm=data.bpm,
            swing=data.swing,
            mixer_channel=mixer_channels[9] if len(mixer_channels) > 9 else None,
            length=data.pattern.length,
            loops=data.loops,
            is_fill=data.fill,
            quantize=data.quantize,
        ...
        if data.pattern.synth2:
            synth2_stem = render_synth(
                ...
                bpm=data.bpm,
                swing=data.swing,
                mixer_channel=mixer_channels[10] if len(mixer_channels) > 10 else None,
                length=data.pattern.length,
                loops=data.loops,
                is_fill=data.fill,
                quantize=data.quantize,
